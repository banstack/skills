...
Work from whatever is already in the conversation context. If the user passes a reference as an argument:

- A path under `docs/prds/*.html` — this is a `to-system-design` PRD artifact. Read the `<script type="application/json" id="prd-data">` block for its structured sections rather than scraping the rendered HTML.
- An issue number, URL, or other tracker reference — fetch it from the issue tracker and read its full body and comments.
